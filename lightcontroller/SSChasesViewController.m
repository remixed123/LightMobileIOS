...
#import "SSChasesViewController.h"
#import "LightData.h"
#import "SSConnection.h"
#import "SSUtilities.h"
#import <QuartzCore/QuartzCore.h>

@interface SSChasesViewController ()

@end

@implementation SSChasesViewController
...
int sliderSpeedChases = 500;
NSString *effectTypeChases = @"0000";
NSString *sortStringValueChases;
NSString *hexColorChases;
NSString *stripeCountChases;
NSString *chaseType;
...
{
    sliderSpeedChases = (int) speedSlider.value;
    
    NSLog(@"adjustSpeed: sliderSpeed: %i",sliderSpeedChases);
}


-(void)setSortType :(NSString*) sortString
{
    sortStringValueChases = sortString;
}
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    predicate = [NSPredicate predicateWithFormat:@"type like %@", sortStringValueChases ];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (NSManagedObject *info in fetchedObjects) {
        ...
        NSLog(@"didSelectRowAtIndexPath: subtype: %@", [info valueForKey:@"subtype"]);
        hexColor = [info valueForKey:@"hexcolor"];
        stripeCount = [info valueForKey:@"stripecount"];
        chaseType = [info valueForKey:@"subtype"];
        hexColorChases = hexColor;
        stripeCountChases = stripeCount;
    }
    
    NSLog(@"didSelectRowAtIndexPath: row: %i: name: %@: subtype: %@",indexPath.row, cellName, cellSubtype ); //  cell.textLabel.text  );
    
    [self sendPacket:hexColor :stripeCount :chaseType ];
    
}


-(void)sendPacket :(NSString*) colorHex :(NSString*) stripeCount : (NSString*) chaseType
{
    int stripeCountInt = [stripeCount intValue];
    NSString *chaseTypeID = @"30";
    //NSString *hexEffectType = effectTypeChases;
    
    
    if ([chaseType isEqualToString:@"Strobe"])
    {
        chaseTypeID = @"30";
        //hexEffectType = colorHex;
        //colorHex = @"";
    }
    else  // @"40"
    {
    ...
    NSString *hexTimeSeperation = [NSString stringWithFormat:@"%@",[utils intToHex2Byte:sliderSpeedChases]];
    ...
    NSString *lwdpPacket = [utils createLwdpPacket:chaseTypeID :payLoad];
