...
float SSColorWheel_PointDistance (CGPoint p1, CGPoint p2)
{
    return sqrtf((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}


PixelRGB SSColorWheel_HSBToRGB (float h, float s, float v)
{
    h *= 6.0f;
    int i = floorf(h);
    float f = h - (float)i;
...
- (PixelRGB)colorAtPoint:(CGPoint)point
{
    ...
    float dist = SSColorWheel_PointDistance(point, CGPointMake(center.x, center.y));
    ...
    return SSColorWheel_HSBToRGB(hue, sat, _brightness);
}
...
- (void)updateImage
{
    ...
    _radialImage = [[UIImage imageWithCGImage:iref] init];
...
{
    PixelRGB pixel = [self colorAtPoint:[self viewToImageSpace:_touchPoint]];
    //[_colorAmount setRedAmount:[[NSNumber numberWithUnsignedChar:pixel.r] intValue]];
    return (pixel.r);
}
...
- (void)setTouchPoint:(CGPoint)point
{
    ...
    if (SSColorWheel_PointDistance(center, point) < _radius)
    {
        _touchPoint = point;
    }
    else
